...
class LenovoPipeline:
    ...
    def process_item(self, item, spider):
        ...
        if activate_mysql == True:
            insert_sql = '''INSERT INTO %s(item_name, price,shopname,comment_num,item_id,start_time,end_time) VALUES('%s',%d,'%s','%s',%d,'%s','%s') on duplicate key update counting=counting+1;'''
            print(insert_sql%(self.search_name,
                  item['name'], float(item['price']), item['shopname'], item['num_comment'], int(item['item_id']),
    ...
    def close_spider(self, spider):
        ...
        with open(filename, 'w', newline='', encoding='gbk') as f:
            ...
            csv_writer.writerow(['item_id','Price', 'Name', 'Shopname', 'commentNum','start_time','end_time'])
